...
| Security (401, 405, 400, 413, rate limit) | `nexql-proto` HTTP (`http.rs`) | Covered |
| Protocol & Tool Dispatching | `nexql-tools` + `scripts/local_mcp_smoke.sh` | Covered |
| Spec Hardening (session issuance/reuse/DELETE) | `nexql-proto` HTTP (`http.rs::tests`) | Covered |
| Spec Hardening (session LRU eviction cap) | Not implemented — session store is unbounded | Open |
