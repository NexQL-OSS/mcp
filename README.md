...
```bash
docker run --rm -i ghcr.io/nexql-oss/mcp:0.3.1 postgres://dev@host.docker.internal:5432/appdb
...
```bash
docker build -t nexql-mcp:0.3.1 .
docker run --rm -i nexql-mcp:0.3.1 postgres://dev@host.docker.internal:5432/appdb
```
