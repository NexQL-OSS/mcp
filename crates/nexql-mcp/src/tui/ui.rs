...
fn draw_header(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let header_lines = vec![Line::from(vec![
        Span::styled(" ⚡ NexQL MCP ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        Span::styled("Setup & Profile Manager", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ...
        Span::styled(format!("{total_profiles}"), Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
        Span::styled(" │ ", Style::default().fg(Color::DarkGray)),
        Span::styled("Default: ", Style::default().fg(Color::DarkGray)),
        Span::styled(default_name, Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
    ])];
    ...
    let line = if let Some(msg) = &app.status {
        ...
        match app.screen {
            Screen::ProfileList => Line::from(vec![
                ...
                Span::styled("[E/Enter]", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ...
            Screen::Summary => Line::from(vec![
                ...
                Span::styled(" Back to Profile Manager", Style::default().fg(Color::White)),
            ]),
        }
    };
...
fn draw_profile_inspector(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let Some(selected_name) = app.profile_names.get(app.selected) else {
        let empty_msg = Paragraph::new(vec![
            Line::from(""),
            Line::from(Span::styled(" No profile selected.", Style::default().fg(Color::DarkGray))),
            Line::from(""),
            Line::from(Span::styled(" Press 'n' to launch the connection setup wizard.", Style::default().fg(Color::Yellow))),
        ])
    ...
    lines.push(Line::from(vec![
        ...
        if is_default {
            Span::styled("  ★ (Default)", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD))
        } else {
            Span::raw("")
        },
    ]));
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(access_label, Style::default().fg(access_color).add_modifier(Modifier::BOLD)),
    ]));
    ...
    lines.push(Line::from(Span::styled("  • Press [E] to modify connection fields", Style::default().fg(Color::DarkGray))));
    lines.push(Line::from(Span::styled("  • Press [T] to test database connection live", Style::default().fg(Color::DarkGray))));
    lines.push(Line::from(Span::styled("  • Press [W] to auto-wire into Cursor, Claude, Zed, VS Code", Style::default().fg(Color::DarkGray))));
    lines.push(Line::from(Span::styled("  • Press [S] to set as default profile", Style::default().fg(Color::DarkGray))));
...
fn draw_profile_form(frame: &mut Frame, app: &App, area: Rect) {
    ...
    lines.push(Line::from(vec![
        Span::styled(name_prefix, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled("Profile Name:     ", Style::default().fg(if name_focused { Color::Cyan } else { Color::DarkGray })),
    ...
    for (i, label) in FIELD_LABELS.iter().enumerate() {
        ...
        let label_style = Style::default().fg(if focused { Color::Cyan } else { Color::DarkGray });
...
fn draw_testing(frame: &mut Frame, app: &App, area: Rect) {
    ...
    match &app.test_outcome {
        TestOutcome::Idle => {
            lines.push(Line::from(Span::styled(" Ready to initiate connection test.", Style::default().fg(Color::DarkGray))));
        }
        TestOutcome::Running => {
            lines.push(Line::from(vec![
                Span::styled(" ⏳ ", Style::default().fg(Color::Yellow)),
                Span::styled("Initiating PostgreSQL connection handshake...", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            ]));
            lines.push(Line::from(""));
            lines.push(Line::from(Span::styled("    Resolving profile credentials and validating socket/TLS parameters...", Style::default().fg(Color::DarkGray))));
        }
        TestOutcome::Done(Ok(report)) => {
            lines.push(Line::from(vec![
                Span::styled(" ✔ ", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
                Span::styled("CONNECTION SUCCESSFUL", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            ]));
            lines.push(Line::from(""));
            lines.push(Line::from(vec![
                Span::styled("   • Handshake Latency: ", Style::default().fg(Color::DarkGray)),
                Span::styled(format!("{:.1} ms", report.latency.as_secs_f64() * 1000.0), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ]));
            lines.push(Line::from(vec![
            ...
            lines.push(Line::from(vec![
                Span::styled("   • Superuser Status:  ", Style::default().fg(Color::DarkGray)),
                Span::styled(
                    if report.is_superuser { "Yes (Warning: elevated permissions)" } else { "No (Standard user)" },
                    Style::default().fg(if report.is_superuser { Color::Yellow } else { Color::Green }),
                ),
            ]));
            lines.push(Line::from(""));
            lines.push(Line::from(Span::styled("   Press [Enter] to proceed and save profile.", Style::default().fg(Color::Yellow))));
        }
        TestOutcome::Done(Err(e)) => {
            lines.push(Line::from(vec![
                Span::styled(" ✖ ", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
                Span::styled("CONNECTION FAILED", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
            ]));
            lines.push(Line::from(""));
            lines.push(Line::from(vec![
            ...
            lines.push(Line::from(""));
            lines.push(Line::from(Span::styled(" Troubleshooting Tips:", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
            lines.push(Line::from(Span::styled("   • Verify host & port are reachable from this machine.", Style::default().fg(Color::DarkGray))));
            lines.push(Line::from(Span::styled("   • Check database name, username, and password.", Style::default().fg(Color::DarkGray))));
            lines.push(Line::from(Span::styled("   • Ensure pg_hba.conf allows connections for this user.", Style::default().fg(Color::DarkGray))));
            lines.push(Line::from(""));
            lines.push(Line::from(Span::styled("   Press [R] to retry, or [Esc] to return to profile editor.", Style::default().fg(Color::Yellow))));
        }
    }
...
fn draw_save_confirm(frame: &mut Frame, app: &App, area: Rect) {
    ...
    lines.push(Line::from(vec![
        Span::styled(" Save profile ", Style::default().fg(Color::White)),
        Span::styled(format!("'{}'", app.form.name), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        Span::styled(" to ", Style::default().fg(Color::White)),
        Span::styled(app.config_path.display().to_string(), Style::default().fg(Color::Cyan)),
        Span::styled("?", Style::default().fg(Color::White)),
    ]));
    lines.push(Line::from(""));
    lines.push(Line::from(Span::styled(" TOML Profile Configuration Preview:", Style::default().fg(Color::DarkGray))));
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(" to confirm save, or ", Style::default().fg(Color::DarkGray)),
        Span::styled("[N / Esc]", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
...
fn draw_confirm_delete(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let lines = vec![
        Line::from(""),
        Line::from(vec![
            Span::styled(" ⚠️  Are you sure you want to delete profile ", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
            Span::styled(format!("'{name}'"), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Span::styled("?", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(Span::styled("    This operation will remove the profile from configuration and cannot be undone.", Style::default().fg(Color::DarkGray))),
        Line::from(""),
        Line::from(vec![
            Span::styled("    Press ", Style::default().fg(Color::DarkGray)),
            Span::styled("[Y]", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
            Span::styled(" to confirm delete, or ", Style::default().fg(Color::DarkGray)),
            Span::styled("[N / Esc]", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|item| {
            ...
            let line = Line::from(vec![
                Span::styled(
                    format!("{checkbox} "),
                    Style::default().fg(if item.selected { Color::Green } else { Color::DarkGray }),
                ),
                Span::styled(format!("{:<30} ", item.display_name), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
...
fn draw_diff_review(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let mut lines = Vec::new();
    lines.push(Line::from(vec![
        Span::styled(" Reviewing Diff ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        Span::styled(format!("({} of {}): ", app.diff_idx + 1, app.diffs.len()), Style::default().fg(Color::DarkGray)),
        Span::styled(entry.display_name, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
    ]));
    lines.push(Line::from(vec![
        Span::styled(" Target File: ", Style::default().fg(Color::DarkGray)),
        Span::styled(entry.config_path.display().to_string(), Style::default().fg(Color::White)),
    ]));
    lines.push(Line::from(""));
    ...
    for line in diff_raw.lines() {
        if line.starts_with('+') && !line.starts_with("+++") {
            ...
            lines.push(Line::from(Span::styled(line, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))));
...
fn draw_summary(frame: &mut Frame, app: &App, area: Rect) {
    ...
    for entry in &app.summary {
        if let Some(err) = &entry.error {
            lines.push(Line::from(vec![
                Span::styled(" ✖ ", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
                Span::styled(format!("{}: ", entry.display_name), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
                Span::styled(err, Style::default().fg(Color::Red)),
            ]));
        } else if entry.skipped {
            lines.push(Line::from(vec![
                Span::styled(" ➖ ", Style::default().fg(Color::DarkGray)),
                Span::styled(format!("{} (skipped)", entry.display_name), Style::default().fg(Color::DarkGray)),
            ]));
            ...
            lines.push(Line::from(vec![
                Span::styled(" ✔ ", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
                Span::styled(format!("{}: ", entry.display_name), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
                Span::styled(format!("Updated {}", path.display()), Style::default().fg(Color::Green)),
                Span::styled(backup_note, Style::default().fg(Color::DarkGray)),
            ]));
        } else if let Some(snippet) = &entry.snippet {
            lines.push(Line::from(vec![
                Span::styled(" 📋 ", Style::default().fg(Color::Yellow)),
                Span::styled(format!("{} (Copy snippet below):", entry.display_name), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            ]));
    ...
    if lines.is_empty() {
        lines.push(Line::from(Span::styled(" No actions performed.", Style::default().fg(Color::DarkGray))));
    }

    lines.push(Line::from(Span::styled(" ✨ Client integration complete! You can now use NexQL MCP in your configured AI tools.", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
